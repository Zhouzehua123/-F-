...
质量域的领域级评分如图\ref{fig:p1_quality_full}所示，柱高和误差线分别表示领域均值与标准差，$n$ 为对应记录数。A1 抽样集与扩展集的对照结果如表\ref{tab:p1_qa}所示。
...
冲突与一致性检验结果如表\ref{tab:p1_conf}所示。图\ref{fig:p1_conf}左侧展示冲突率最高的 12 对指标关系，边旁数值为冲突率，线宽与其成正比；右侧列出全部 22 项熵权。M01--M22 按表\ref{tab:p1_dir}的指标顺序编号，紫色与青绿色分别对应列表型与标量型指标。
...
两套质量口径（A1--A3 的熵权质量 $Q$ 与损失难度代理 $Q^{(\text{loss})}$）在 6 个可映射域上的排序一致性如图\ref{fig:p1_quality}所示。右侧连线连接同一领域在两套口径下的名次，名次 1 表示得分最高。
