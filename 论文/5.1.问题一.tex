...
综合质量采用哪种评分方式，需要结合解释性和计算结果选择。表\ref{tab:algo1}比较了熵权法、等权平均及 PCA 第一主成分。评价分歧则分开考察：成对标准化得分差回答一条文本在哪些指标上有分歧，Kendall 协同系数回答整体排序是否一致。配比拟合另外比较普通最小二乘、岭回归和随机森林。
...
等权与熵权给出的结果有相近之处，也有差别。表\ref{tab:algo1}中，两者样本得分的相关为 $0.854$，领域排序的 Spearman 系数为 $0.679$，说明总体趋势相近并不保证每个领域的名次相同。本文采用熵权法，以样本中的信息差异确定权重，并继续检验改换权重的影响。PCA 虽能减少相关信息、压缩维度，其第一主成分却不容易直接说明文本质量，故未作为最终评分。

配比回归更需要注意系数稳定性。17 项占比之和为 1，增加其中一项就必须减少其他项，普通回归可能因此出现不稳定估计。岭回归加入 $L_2$ 惩罚后，既可缓解这一问题，也保留了线性系数的解释。对于指标冲突，同样不宜只看一个数，因此样本分歧与整体一致性分别报告。
...
本问把质量评价与配比计算分别安排在 A1--A3 和 A4--A15 上。用于评分的 $272{,}505$ 条记录中，$51{,}230$ 条来自 A1 抽样集，其余 $221{,}275$ 条为 A2、A3 的扩展记录。
...
先看两套记录是否给出接近的评分。arxiv、github 均有抽样和扩展数据，表\ref{tab:p1_qa}中两者之差依次为 $+0.0021$、$+0.0006$。这两个差值都较小，说明在可直接比较的领域中，抽样集与扩展集的平均评分接近。

全量记录的最高均分来自 arxiv，为 0.703，最低为 github 的 0.455。其间依次是 stackexchange 的 0.655、c4 的 0.637 和 commoncrawl 的 0.615；book 为 0.552，与 wikipedia 的 0.551 很接近。这一排序反映的是 22 个分类器信号共同给出的评价。例如，rps\_\allowbreak{}doc\_\allowbreak{}frac\_\allowbreak{}unique\_\allowbreak{}words 与 rps\_\allowbreak{}doc\_\allowbreak{}unigram\_\allowbreak{}entropy 都会受到文档长度影响。因此，网页类得分高于书籍、百科，并不意味着它们在所有训练任务中都更有价值。
...
这些指标只能说有一定共同倾向。全量记录的 Kendall 系数为 $0.069$，一致程度较弱；$\chi^2$ 检验虽有 $p<0.001$，但在大样本下显著并不代表一致性很强。冲突率的对照提供了另一面证据：实际为 $22.3\%$，独立参照为 $28.9\%$，二者比值 $0.774$。因此，指标间既有共同判断，也保留了较多分歧。
...
跨规模比较时，先要注意损失基准变了。60M 相对 1M 的损失约为 $0.68$ 倍，若直接套用原来的损失尺度，误差评价会受到影响。于是另看 Pearson 相关，60M、1B 检验的平均值为 $0.782$、$0.651$。配比变化对应的部分趋势仍然存在，但预测绝对损失时还需调整尺度。
...
没有占比上限时，线性优化会把配方集中到一个低损失领域，预测加权损失可到 $4.177$。这种过于集中的推荐缺乏均衡性，故把各域上限限制为参考占比的 2.5 倍。表\ref{tab:p1_rec}中，有界方案的损失为 $5.136$，参考方案为 $5.336$；依据未舍入值计算，改善量为 $0.199$。
