...
\]
该值代表参考配方原有的质量，全量文本均值 $0.4912$ 则留作灵敏度对照。用 $Q_A$ 表示问题一评分、$Q_B$ 表示附件 B 的质量轴，两者的接口为 $Q_B=h(Q_A)$。本问采用恒等映射 $h(q)=q$，同时将费用函数定义在该共同数值轴上；这是一项未由配对标定数据识别的衔接假设，并非同向评分天然等价。优化只考虑 $Q_0\le Q\le1$，其中 $Q=1$ 是模型质量上界，不表示不同来源的满分已获等价验证。

我们以 $Q_0$ 同时作为优化起点和计费参照，只对超过参考质量的部分计算增量费用。领域评分经配比加权后，以单一质量变量进入损失与成本函数。因此，模型给出整体质量与规模的配置关系，尚未细分各领域的清洗、筛选或标注操作；用于语料处理时，仍需将整体目标落实为领域处理方案。配比情景也固定共同基线 $Q_0$，不重新计算 $Q_0(\bm p)$，所以这里分别考察损失平移与质量投入，并未求解配方、基线和处理成本同时联动的优化。

为控制情景间的差异，我们先以 $M=0$ 的参考配比、幂函数型质量成本和 $2048$ tokens 上下文作基准；之后每次只更换质量成本或上下文。上下文情景依据附件 C7 的架构记录设置：2048、4096、8192、32768、131072 tokens 分别作为给定条件代入预算式，用于比较同一预算在不同上下文要求下的配置。求解器在每个情景内调整参数、数据和质量，不把上下文长度作为自由决策变量。此外，把问题一推荐配比对应的 $M=-0.1730$ 单列为情景，观察预测损失的变化。
...
领域权重同样影响预训练效率\cite{xie2023doremi}。在本模型中，它通过固定项 $M$ 体现：在 $10^{22}$ FLOPs 预算下，替换为问题一的推荐配比后，$N,D,Q$ 均不变，而预测损失由 2.1451 降至 1.9721。若保持参考配比，用现有二分计算达到同一损失，约需 $1.691\times10^{23}$ FLOPs，约为原预算的 16.9 倍。该倍数由固定损失平移假设得出，表示模型内的等效预算；若该假设幅度仅保留 $0\%$、$25\%$、$50\%$、$75\%$，同一计算给出 $1.00$、$1.81$、$3.49$、$7.30$ 倍。这些保留比例是敏感性情景，没有概率含义；要跨规模使用仍须满足问题二的迁移条件，不能把它当成实际训练加速比。
