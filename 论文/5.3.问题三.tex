...
数值计算采用 SLSQP 求解式\eqref{eq:p3_problem}，以 $\ln N$、$\ln D$ 缩小变量尺度差异；停止条件取 ftol=$10^{-14}$，单次求解最多迭代 1500 次。三档预算各设 8 组初值，连续预算扫描各设 5 组，从求解成功的结果中选取损失最低的一组，记作 $(N^*,D^*,Q^*)$。这样可降低初值和局部解的影响，但不能保证全局最优。另因 $\gamma<1$ 时 $Q=1$ 的导数无界，将达到该上界的配置作为边界解处理。将保存的配置重新代入成本函数，三档预算的最大相对预算残差约为 $1.0\times10^{-15}$，上下文敏感性结果的最大残差约为 $2.4\times10^{-15}$；预算扫描的 140 个有效点中最大残差约为 $1.9\times10^{-14}$。这些数值核验说明记录的配置满足预算约束，但只验证可行性，不构成对全局最优性的证明。
