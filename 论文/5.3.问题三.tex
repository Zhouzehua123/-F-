...
\]
该值代表参考配方原有的质量，全量文本均值 $0.4912$ 则留作灵敏度对照。记问题一评分为 $Q_A$、附件 B 评分为 $Q_B$，本问基准情景明确采用恒等映射 $Q_B=h(Q_A)=Q_A$，并将其共同记为 $Q$。这一设定使现有损失与题设费用可以在同一评分轴上计算，尚未获得跨附件标定的支持。优化范围为 $Q_0\le Q\le1$，其中 $Q=1$ 是模型上界，不表示不同来源的满分已经证实为同等质量。

我们将参考 $Q_0$ 外生固定为优化起点和计费参照，只对超过该水平的部分计算增量费用。领域评分经配比加权后，以单一质量变量进入损失与成本函数，尚未细分各领域的处理操作。配比条件算例也保持这一共同费用基准，以单独观察固定损失平移的作用；它不表示换配方后未处理语料的实际质量仍然相同。按同一领域映射，线性条件候选配方自身的质量为 $0.5354$。若按各配方的原料质量分别计费，则应使用 $Q_0(\bm p)$，需要另行处理配比与质量成本的耦合。

为控制情景间的差异，我们先以 $M=0$ 的参考配比、幂函数型质量成本和 $2048$ tokens 上下文作基准；之后每次只更换质量成本或上下文。上下文情景依据附件 C7 的架构记录设置：2048、4096、8192、32768、131072 tokens 分别作为给定条件代入预算式，用于比较同一预算在不同上下文要求下的配置。求解器在每个情景内调整参数、数据和质量，不把上下文长度作为自由决策变量。此外，把问题一线性条件候选配比对应的 $M=-0.1988$ 单列为算例，检查假定收益幅度如何影响损失与等效预算。
...
\begin{equation}
 ...
 \label{eq:p3_problem}
\end{equation}
末尾两项界限用于控制数值搜索范围，并不表示实际数据供给的上限。在共同参考 $Q_0$、配比可行域独立且 $M(\bm p)$ 可加分离的简化模型中，先选取使 $M$ 最小的配比、再优化 $N,D,Q$，可与相应的联合最小化等价。本节固定给定配比作条件比较；问题一线性条件候选在其代理模型约束下的最优性，不等同于真实训练中的最优配方。若 $Q_0$ 或费用随配比变化，上述分离不再成立。因此，算例中的“配置不变”来自共同基准和加性结构，不能视为配比与规模实际上相互独立的证据。
...
领域配比可以影响预训练效果\cite{xie2023doremi}，但问题一的两个代理对当前候选的收益方向存在分歧，本节不将该候选作为实际节约算力的推荐。仅按线性条件值 $M=-0.1988$、共同参考基线和固定损失平移作算例：在 $10^{22}$ FLOPs 下，$N,D,Q$ 不变，预测损失由 2.1451 降至 1.9463。参考配比达到后一损失的模型等效预算约为 $2.963\times10^{23}$ FLOPs，即 29.6 倍。这个数值保留的是原模型计算，不是实测加速比。

为观察绝对收益假设的影响，将 $-0.1988$ 分别保留 $0$、$25\%$、$50\%$、$75\%$ 和 $100\%$，在当前满分配置下反解参考配比的等效预算，结果见表\ref{tab:q23_review_m_amplitude}。只保留一半幅度时，倍数降至 4.31；因而等效预算对损失平移的绝对大小十分敏感。这些比例是条件设定，不是收益估计的置信区间，不能据此保证任何实际节约量。
...
\begin{table}[htbp]
 ...
 \toprule
 线性收益保留比例 & 0 & $25\%$ & $50\%$ & $75\%$ & $100\%$ \\
 \midrule
 等效预算倍数 & 1.00 & 1.98 & 4.31 & 10.49 & 29.63 \\
 \bottomrule
 \end{tabularx}
\end{table}

配比情景与成本情景作用于模型的不同位置。固定的配比修正只改变目标函数的常数项，对参数、数据和质量求导时均消去，因此在同一成本约束下保留原有配置。更换质量成本或上下文则会改变预算边界，即使损失函数的估计系数不变，也需要重新比较可行配置。作为基线口径的对照，仅将候选配方的 $Q_0=0.5354$ 代入费用时，$10^{22}$ FLOPs 下的满分配置变为约 6.097B 参数、229.0B tokens，与参考基线的差异较小，但已不再完全相同。该对照单独保存，不替换前述共同基准下的结果。
