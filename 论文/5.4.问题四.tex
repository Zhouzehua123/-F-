...
前三问围绕训练损失展开，本问把视线转到模型在实际评测中的表现。已有研究分别讨论了规模增加时的能力涌现\cite{wei2022emergent}，以及评分方式造成的表观突变\cite{schaeffer2023mirage}。本文先检查损失和得分是否保持反向关系，再把历史增长拆成规模变化与其他变化两部分，最后估计未来的能力前沿。由于评测体系和样本来源并不完全一致，综合分数与单项任务结果同时保留：前者用于比较整体水平，后者用于寻找具体短板。
...
\begin{equation}
    \bar B = a + b\,L
\end{equation}
C6 的高可比层主要由同模型、同验证集的 Pythia 记录构成，可与附件 B 对照；中可比层来自不同模型族和技术报告，验证集并不统一。两层分别采用最小二乘拟合，再把合并结果与 C5 对照，使来源差异留在结果中，而不是将所有记录视作同一实验。
...
图\ref{fig:p4_map}和表\ref{tab:p4_bridge}给出的结论比较有限：损失低时得分通常较高，但不是可以精确换算的直线。合并样本的 Pearson $r=-0.510$、Spearman $\rho=-0.549$，$R^2=0.26$，MAPE 为 $67.8\%$。中可比层的验证集不一致，低分还会放大相对误差；高可比层 MAPE 虽为 $5.6\%$，样本只有 7 条，得分范围也仅 $5.07\sim6.06$。因此，这一小组记录不足以说明高能力模型同样符合该映射。

后续只保留“损失下降与能力上升大体同向”这一层含义，不把损失改善换算成确定的能力分，也不由跨模型相关推断某项资源调整的因果效果。历史分解和前沿预测直接使用 Benchmark 得分，避免继续传递桥接误差。
...
全样本的规模和年度系数为 13.842、4.844；预训练类为 6.414、1.552；对话及微调类为 14.853、4.680。类型间差异提醒我们，回归结果受到样本构成影响。由于没有同一模型微调前后的受控对照，不能直接把差异归因于微调效率；线性式在很小参数端还可能给出负分，因此这里只将它作为规模关系的概括和增量核算工具。
...
C3 清理时发现 13 条记录的 Average 与六维均分相差超过 5 分，故将其剔除；最大差约 42.3 分，来自 GPT-3 175B（Average 50.0，六维均分 7.67）。清理后，长期回归的 $b_P=14.10$、$b_t=3.30$、$R^2=0.42$。2019--2025 年累计前沿由 5.00 升至 52.08，总增量 47.08 分，其中规模项 23.77 分、非规模项 23.31 分，占比为 50.5\%、49.5\%。由于历史评分来源不完全相同，这些数值主要用于比较长期量级。
...
\end{equation}
曲线估计为 $K=56.61$、$r=2.523$、$t_0=5.11$，拟合 $R^2=0.975$。残差采用 600 次 bootstrap 重抽样，取成功重拟合预测值的第 5 和第 95 百分位作为 $90\%$ 区间。这个区间只反映既定曲线和样本重抽样的不确定性，尚未包含评测标准或未来技术路线改变的影响。
...
IFEval 的最高分为 89.98，距满分约 10.0 分，但中位数只有 43.51，说明少数领先模型较强，整体并未饱和。GPQA、MUSR 最高仅 27.74、38.69，中位数更低，是当前较明显的短板。MATH Lvl 5 也有类似分化：最高 62.54，而中位数为 9.93，前沿改善尚未普遍传到大多数模型。
...
C4 全量 3,523 条记录中，1,390 条有算力值，1,424 条有训练数据量；权重状态明确的 2,653 条里，Yes 为 1,325、No 为 1,328。训练数据量缺失较多且来源不统一，本节只报告覆盖情况，不据此估计年度增长。宏观记录可帮助设置资源情景，却不能说明算力放缓会让非规模进步按哪一个比例下降。

本问先确认损失和能力大体反向变化，再把长期和近期前沿的增量分别核算。长期窗口中，规模与非规模贡献接近各半；2024 年 6--9 月的短窗口则主要由非规模项构成。未来预测同时保留曲线区间和结构情景，任务层面则提示数学、研究生级问答和多步推理仍是需要重点观察的薄弱部分。
