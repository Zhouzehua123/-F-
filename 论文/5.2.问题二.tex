...
B6、B7 的平均相关分别为 $-0.925$、$-0.918$，两者均可直接使用 $Q=Q_{\text{score}}$。但它们不是两份独立证据：逐行核对后，B7 包含 B6 全部 360 条记录，真正新增的只有 90 条。B8 的两个子集则与 B6/B7 方向相反，还有部分损失低于模型下限，故不参与主拟合，仅把评分翻为 $1-Q_{\text{score}}$ 后检查偏差。

图\ref{fig:p2_direction}把这一差异画得更直观。每个圆点为一组固定 $(N,D)$ 的相关，菱形为组间平均；B6/B7 的点多数位于零点一侧，B8 多数位于另一侧，可见差异并非个别组造成。另外，B6 与 B7 点群接近部分来自记录重复，不能把相似图形再算作一次独立验证。
...
\begin{samepage}
表\ref{tab:p2_form}中，四种写法对 B7 新增点的相关都达到 $0.983$ 以上，单看相关很难取舍。进一步比较联合样本与 B6 的 $R^2$，加性质量缺口形式均为最高，故采用这一形式。后面还会给出 RMSE、MAPE，以判断趋势接近时，预测值究竟偏离了多少。
\par
\end{samepage}
...
\end{equation}
在 Pythia 族内，留出较大模型后预测仍较准确。只用最小的 4 个模型重估、预测另外 4 个，得到 $R^2=0.9999997$、MAPE $=0.005\%$。作为对照，全部记录拟合为 $R^2=0.9999998$，RMSE $=1.5\times10^{-4}$，MAPE $=0.004\%$。两种结果接近，说明所考察族内的规模关系较稳定。
\par
\end{samepage}
...
B1 的残差标准差为 $1.5\times10^{-4}$，而损失标准差为 $0.342$，函数几乎贴合这组记录。这样的吻合程度只对应当前来源，不能直接推广到其他训练资料。因此，下一步仍以 B4、B5 的外部记录及 B2 半合成轨迹作对照。
...
\begin{samepage}
跨来源检验中的主要问题是预测值仍有偏差。表\ref{tab:p2_val}给出 B4 的 MAPE 为 $8.8\%$、相关为 $0.975$，B5 的 MAPE 为 $8.0\%$、相关为 $0.910$。也就是说，模型能描述一部分跨族变化趋势，但趋势接近不代表每个损失值都准确。
\par
\end{samepage}
...
\begin{samepage}
B2、B8 的偏差明显较大。前者 MAPE 为 $33.7\%$，后者校准段即使翻转质量评分，仍为 $133.4\%$，且部分损失低于模型渐近下限。可见问题不止是分数方向；损失尺度和数据生成方式不同，也会使这些来源无法直接合并使用。
\par
\end{samepage}
...
\begin{samepage}
表中较小参考点 $(N_0,D_0,Q_0)=(1,100,0.6)$ 的质量弹性绝对值约为参数弹性的 $1.7$ 倍，同样小比例的改善中，提高质量对应的损失降幅较大。把视线移到两个参考点的替代率，可得 $2.844$ 与 $76.838$ B/单位质量；若用一阶式代入 $0.1$ 的质量增量，分别相当于增加 $0.284$B、$7.684$B 参数。这组数值用于说明起点规模的影响，有限变化仍应按式\eqref{eq:p2_finite}计算，并检查拟合范围。它们表示损失效果相近，不表示成本相同。
\par
\end{samepage}
