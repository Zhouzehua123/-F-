...
$N$ & 模型包含的参数数量\\
$D$ & 用于模型训练的数据总量\\
$L,E$ & 验证交叉熵损失、参考配比下的渐近损失基准\\
$Q$ & 当前模型采用的质量变量，分值越高则质量越好\\
$Q_A,Q_B,h$ & 附件 A 的综合评分、附件 B 的实验质量变量及假设映射 $Q_B=h(Q_A)$\\
$Q_0$ & 质量基线；问题三沿用既定参考配比的加权质量，作为外生成本基线\\
$\bm p$ & 各领域训练占比构成的向量，各项非负且总和为 1\\
$\theta,\lambda_k$ & 配比核函数的距离参数与核岭正则强度\\
...
$F(t),K$ & 能力前沿及当前窗口拟合的 logistic 渐近水平\\
$t,t_0$ & 以 2019 年为原点的时间、logistic 拐点时间\\
$r$ & logistic 增长率\\
$b_P$ & 参数量每扩大十倍对应的回归得分变化\\
$b_t$ & 控制规模后，不同年份记录的条件得分差异系数\\
$\alpha_t$ & 合并横截面回归的年份控制项\\
