...
\begin{longtable}{>{\centering\arraybackslash}p{0.20\textwidth}>{\centering\arraybackslash}p{0.76\textwidth}}
  ...
  \bottomrule
  \endfoot
  $N$ & 模型参数量（问题二以 B 为单位；问题三成本计算使用参数个数） \\
  $D$ & 训练数据规模（问题二以 B tokens 为单位；问题三成本计算使用 token 数） \\
  $L$ & 验证交叉熵损失（越低越好） \\
  ...
  $C,\gamma$ & 广义标度律中质量缺口项的系数与指数（问题三将系数记为 $C_q$） \\
  ...
  $C_{\text{budget}},C_{\mathrm b}$ & 算力预算（单位：FLOPs；问题三简记为 $C_{\mathrm b}$） \\
  ...
  $\tau(Q)$ & 质量处理成本折算的等效附加参数规模，与问题三的 $N$ 同口径 \\
  $\Theta(C_{\mathrm b})$ & 质量提升完成度；问题三以其对 $\log_{10}C_{\mathrm b}$ 的变化率描述提升速度 \\
