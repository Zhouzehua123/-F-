...
ROOT = HERE.parents[1]
BLUE, TEAL, RED, GRAY = '#665080', '#367E73', '#CB8B38', '#625C69'
...
def redesign(py,b6,b7,b7n,b8,b8c,read,model,classic,E,A,a,B,b,C,g,save):
    ...
    def foot(fig,s): fig.text(.02,.012,s,fontsize=9.5,color=GRAY,ha='left')
    ...
    cb=fig.colorbar(im,cax=ax.inset_axes([0,1.08,1,.045]),orientation='horizontal');cb.ax.tick_params(labelsize=9,length=2)
    ax.set_title('(a) 附件记录的损失地图',loc='left',pad=43,fontsize=12)
    residuals=[]
    for n in ns:
    ...
    ar.set_title('(b) 各参数规模的拟合残差',loc='left',pad=43,fontsize=12);style(ar)
    fig.subplots_adjust(left=.09,right=.985,bottom=.23,top=.75)
    foot(fig,'B1：8 个模型 × 147 个检查点；左图颜色表示验证损失。\n右图：点为记录，粗线为四分位区间，金点为中位数；分布宽度表示平滑密度。')
    save(fig,'图1_经典标度律拟合')
    ...
    for j,(name,kind,df,q,col) in enumerate(sets):
        ...
        ar.text(mape*1.17,j,f'{mape:.1f}%',va='center',fontsize=10,color=col);ar.text(1.35,j,f'{r:.3f}',transform=ar.get_yaxis_transform(),va='center',ha='center',fontsize=10)
    ...
    ax.set_xscale('symlog',linthresh=10);ax.set_xticks([-50,-10,0,10,50,200,500]);ax.set_xticklabels(['−50','−10','0','10','50','200','500'],fontsize=9)
    ...
    foot(fig,'左轴为对称对数轴，±10% 内线性；误差 = 100 × (预测值 − 给定值) / 给定值。\nB8 使用 1−Q 方向变换；空心点为中位数，粗线为四分位区间。');save(fig,'图2_广义标度律验证')
    ...
    fig.subplots_adjust(left=.085,right=.98,bottom=.23,top=.85);foot(fig,'左图固定 D=300 B tokens；空心点为 B6 半合成实验位置，底色与等值线为模型预测。');save(fig,'图3_质量边际影响')
    ...
    for ax,effect,col,labels,xlabel,heading in [
        ...
        ypos=np.arange(5);base=effect.min()*.8 if ax is axs[0] else .34
        ax.hlines(ypos,base,effect,color=col,lw=2);ax.scatter(effect,ypos,color=col,s=35,zorder=3)
    ...
    fig.subplots_adjust(left=.075,right=.98,bottom=.28,top=.85,wspace=.52)
    foot(fig,'边际导数分别采用各自量纲；弹性为无量纲指标。参考点均取 Q=0.6，N、D 以 B 为单位。');save(fig,'图4_边际效用对比')
    ...
    for ax,target,col in zip(axs,[2.30,2.10,1.95],[BLUE,TEAL,RED]):
        ...
        if .07<min_n<1e4:
            ...
            ax.annotate(f'Q=1\nN={min_n:.1f} B',(min_n,1),xytext=pos,textcoords='axes fraction',fontsize=10,color=col,ha='center',va='top',arrowprops=dict(arrowstyle='-',lw=.7,color=col))
        if target==1.95:
            ax.set_ylim(.95,1.008);ax.set_yticks([.96,.98,1]);ax.text(.06,.1,'高质量区间放大',transform=ax.transAxes,fontsize=10,color=col)
    fig.subplots_adjust(left=.08,right=.985,bottom=.24,top=.82,wspace=.38)
    foot(fig,'D=300 B tokens。实线：N≤12 B；虚线：规模外推。右图单独放大纵轴；阴影区分拟合规模与外推规模。');save(fig,'图5_质量参数替代')
    ...
    foot(fig,'依据问题一配比系数；聚类距离为 √(2−2s)。绿色：相对效应趋同；金色：相对效应相反。\n右图分别选取 s 最大与最小的 4 对；相似性不直接表示替代效率或互补增益。');save(fig,'图6_领域替代互补')
    ...
    fig.subplots_adjust(left=.21,right=.98,top=.84,bottom=.25)
    foot(fig,'圆点：一个 (N,D) 组；菱形：组间算术平均。均为半合成数据；B7 含 B6，不构成独立证据。');save(fig,'图7_质量方向对照')
