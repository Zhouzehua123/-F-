...
输出字段 stationarity_gap 沿用原名称，实际记录的是“数值保护梯度间隙”：
零份额处以 1e-30 保护平方根导数；success 仅表示该停止量达阈值。
它与约束残差共同用于数值筛选，不构成边界光滑驻点或全局最优证书。
"""
...
def recommend(data, result_dir, output_dir):
    ...
    def value_gradient(z):
        # 仅在正份额内为通常的光滑梯度；零份额用数值保护，不据此证明驻点。
    ...
    for j,start in enumerate(starts):
        ...
        for iteration in range(150):
            ...
            direction=vertex.x-z;gap=float(-gradient@direction)  # 数值保护梯度间隙
            if gap<1e-6:break
